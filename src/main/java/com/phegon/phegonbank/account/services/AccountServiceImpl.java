...
public class AccountServiceImpl implements AccountService {
 ...
 @Override @Transactional public Response<AccountDTO> updateStatus(String n,String status){Account a=find(n);ensureOwner(a);AccountStatus s=AccountStatus.valueOf(status.toUpperCase());if(a.getStatus()==AccountStatus.CLOSED)throw new BadRequestException("Closed accounts cannot be changed");if(s==AccountStatus.CLOSED && a.getBalance().compareTo(BigDecimal.ZERO)!=0)throw new BadRequestException("Account must have zero balance before closing");if(s==AccountStatus.CLOSED)a.setClosedAt(java.time.LocalDateTime.now());a.setStatus(s);repo.save(a);return ok(a,"Account status updated");}
