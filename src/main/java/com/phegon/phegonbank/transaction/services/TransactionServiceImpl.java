...
public class TransactionServiceImpl implements TransactionService {
 private final AccountRepo accounts; private final TransactionRepo transactions; private final ModelMapper mapper;
 @Override @Transactional public Response<TransactionDTO> execute(TransactionRequest r){
  ...
  Account source=account(r.getAccountNumber()); owner(source);
  active(source);
  if(r.getTransactionType()==TransactionType.DEPOSIT){source.setBalance(source.getBalance().add(r.getAmount()));return save(source,r,source.getAccountNumber(),null);}
  if(r.getTransactionType()==TransactionType.WITHDRAWAL){debit(source,r.getAmount());return save(source,r,source.getAccountNumber(),null);}
  if(Objects.equals(source.getAccountNumber(),r.getDestinationAccountNumber()))throw new InvalidTransactionException("Source and destination accounts must differ");
  Account dest=account(r.getDestinationAccountNumber());active(dest);debit(source,r.getAmount());dest.setBalance(dest.getBalance().add(r.getAmount()));accounts.save(dest);
  Response<TransactionDTO> result=save(source,r,source.getAccountNumber(),dest.getAccountNumber());
  Transaction credit=Transaction.builder().amount(r.getAmount()).transactionType(TransactionType.TRANSFER).transactionDate(java.time.LocalDateTime.now()).description(r.getDescription()==null?"Transfer received":r.getDescription()).status(TransactionStatus.SUCCESS).account(dest).sourceAccount(source.getAccountNumber()).destinationAccount(dest.getAccountNumber()).build();
  transactions.save(credit); return result;
 }
 @Override public Response<List<TransactionDTO>> history(String n,int page,int size){Account a=account(n);owner(a);var p=transactions.findByAccount_AccountNumber(n,PageRequest.of(Math.max(page,0),Math.min(Math.max(size,1),100)));return Response.<List<TransactionDTO>>builder().statusCode(200).message("Transactions fetched").data(p.getContent().stream().map(t->mapper.map(t,TransactionDTO.class)).toList()).meta(java.util.Map.of("page",p.getNumber(),"size",p.getSize(),"totalElements",p.getTotalElements(),"totalPages",p.getTotalPages())).build();}
