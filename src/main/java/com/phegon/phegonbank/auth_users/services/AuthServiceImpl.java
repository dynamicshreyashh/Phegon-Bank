...
public class AuthServiceImpl implements AuthService {
    ...
    public Response<LoginResponse> register(RegistrationRequest request) {
        ...
        Role customer=getOrCreateRole("ROLE_CUSTOMER");
        User user=User.builder().firstName(request.getFirstName().trim()).lastName(request.getLastName())
                ...
                .active(true).roles(List.of(customer)).build();
    ...
    @Override public Response<UserDTO> getCurrentUser() {
        ...
        return Response.<UserDTO>builder().statusCode(200).message("User fetched").data(modelMapper.map(user,UserDTO.class)).build();
    }
    ...
    private User currentUser() {
        if(SecurityContextHolder.getContext().getAuthentication().getPrincipal() instanceof AuthUser au)
            return userRepo.findById(au.getUser().getId()).orElseThrow(()->new NotFoundException("User not found"));
